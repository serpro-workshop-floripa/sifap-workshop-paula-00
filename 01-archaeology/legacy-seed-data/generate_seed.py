...
def write_readme(counts):
    # Prose follows the edition language declared in .github/language.json.
    text = f"""# Synthetic legacy dataset

> **Path:** [Team Kit](../../README.md) › [Stage 1](../README.md) › **Synthetic legacy dataset**

**The synthetic records the legacy SIFAP source is populated from.** This folder contains the fixed-width files the authorized source owner loads into Adabas before Stage 1. Teams decode them to learn the record layouts and to prepare independent expectations for validating the modern system; the migration itself starts from the approved Adabas snapshot.

| Field | Value |
|---|---|
| **Target audience** | Pair 4 (DBA + QA); Pairs 1 and 3 consult it for population, test, and acceptance planning |
| **Prerequisites** | Read the DDMs in [`legacy-sifap/adabas-ddms/`](../legacy-sifap/adabas-ddms/) and the [data migration guide](../../docs/DATA-MIGRATION.md) |
| **Estimated time** | 15 min |
| **Stage** | Preparation and Stage 1; reference for Stage 3 tests and reconciliation |
| **Expected outcome** | Decoded layouts, recorded population provenance, and independent expectations for validating the modern system |
...
## Role in the data migration

| Use | Rule |
|---|---|
| Population provenance | The DBA records this dataset as the synthetic-data provenance in source readiness, then measures the populated Adabas source. The counts below are intended counts, not proof of what the source currently contains. |
| Migration source | Not this folder. Stage 3 loads PostgreSQL from the approved Adabas snapshot through the supported extraction route. Loading these files directly into PostgreSQL, or generating a new seed, is not a migration and does not meet H3. |
| Independent expectations | QA may decode these records to prepare expectations, such as the beneficiary key set, related payments, and exact benefit and payment values, and cross-check them with the snapshot and PostgreSQL. A difference is a finding to record, not a value to fix silently. |
| Test fixtures | Developer and QA may derive isolated test inputs from the learning fixtures below. Fixtures support tests; they never replace the population in the acceptance path. |

See [source to target, not seed to seed](../../docs/DATA-MIGRATION.md#source-to-target-not-seed-to-seed) in the data migration guide.
...
Use these records as candidate inputs for equivalence and validation tests. Derive expected values from the reviewed Natural logic, not from the generator: some rows are deliberately inconsistent.
