...
Author and year below are transcribed from each member's own `* AUTHOR:` and
`* DATE:` header lines, and are kept in step with [`CHRONOLOGY.md`](../CHRONOLOGY.md)
by the chronology CI gate. When a document elsewhere in the corpus states a
different author or year, you have found evidence, not a typo — see
[declared drift](../DECLARED-DRIFT.md).

| Pair | Program | Author in header | Created | Description |
|---|---|---|---|---|
| **1 · Vision** (PO + RE)—registration | `CADBENEF.NSP` | Carlos Roberto da Silva | 1997 | Beneficiary registration—creation, update, deletion |
| | `CADDEPEN.NSP` | Ana Lucia Pereira | 1998 | Registration of a dependent linked to the primary beneficiary |
| | `CADPROG.NSP` | Marcos Antonio Ribeiro | 1997 | Social program registration—parameters and value ranges |
| **2 · Architecture** (EA + SA)—batch | `BATCHPGT.NSP` | Carlos Roberto da Silva | 1997 | Batch payment—generates monthly payment cycles |
| | `BATCHREL.NSP` | Patricia Gomes de Souza | 1999 | Batch report—produces management reports |
| | `BATCHCON.NSP` | Marcos Antonio Ribeiro | 2000 | Batch reconciliation—reconciles payments with bank returns |
| **3 · Implementation** (TL + Dev)—calculation | `CALCBENF.NSN` | Carlos Roberto da Silva | 1997 | Calculates the benefit amount by program and range |
...
| **4 · Quality** (DBA + QA)—validation | `VALBENEF.NSN` | Marcia Helena Oliveira | 1998 | Validates registration data (CPF, NIS) |
| | `VALDOCS.NSP` | Ana Lucia Pereira | 1998 | Validates CPF, NIS/PIS, RG, CTPS and supporting documents |
| | `VALELEG.NSN` | Jose Ferreira dos Santos | 1999 | Validates eligibility against program rules |
