...
![Legacy](https://img.shields.io/badge/Legacy-SIFAP%201.0-171717?style=flat-square) ![Dated sources](https://img.shields.io/badge/Dated%20sources-1997--2018-404040?style=flat-square) ![Language](https://img.shields.io/badge/Language-Natural%2BAdabas-737373?style=flat-square)

> [!IMPORTANT]
> **This is an in-fiction document, not kit truth.** It is the SIFAP technical
> dossier as the support team would have assembled it, and it therefore carries
> the attribution errors, retro-fitted release notes, and partial inventories
> that real dossiers carry. Where it disagrees with a source header, the header
> wins. For the dates the kit itself stands behind, read
> [`CHRONOLOGY.md`](CHRONOLOGY.md); for the divergences that are deliberate,
> read [declared drift](DECLARED-DRIFT.md). Treat every claim below as a lead to
> verify against `natural-programs/` and `adabas-ddms/`, never as a settled fact.
...
> [!WARNING]
> The timeline groups work by the project that funded it, which is how release
> notes are usually written and why four of its rows disagree with the creation
> dates in the members themselves (`DRIFT-01` to `DRIFT-04` in
> [declared drift](DECLARED-DRIFT.md)). The evidence-derived release chronology
> is in [`CHRONOLOGY.md`](CHRONOLOGY.md#5-canonical-release-chronology).
...
> [!WARNING]
> This roster is institutional memory, and it does not match the names recorded
> in the source headers. Registered as `DRIFT-05` and `DRIFT-08` in
> [declared drift](DECLARED-DRIFT.md); the name index that resolves the two
> spellings of a single author is in [`CHRONOLOGY.md`](CHRONOLOGY.md#6-canonical-name-index).
...
> [!WARNING]
> The author, year, and last-change columns below come from the 2008
> documentation effort, not from the members themselves. Several rows disagree
> with the `* AUTHOR:` and `* DATE:` headers in `natural-programs/`. This is
> registered as `DRIFT-06` and `DRIFT-07` in [declared drift](DECLARED-DRIFT.md).
> Use [`CHRONOLOGY.md`](CHRONOLOGY.md) when you need the attribution the kit
> stands behind, and open the member when you need the truth.
...
| Previous | Next |
|---|---|
| [Stage 1 GUIDE](../GUIDE.md)<br/><sub>Timed 90-minute walkthrough.</sub> | [Chronology](CHRONOLOGY.md)<br/><sub>The dates, authors, and name index the kit stands behind.</sub> |
