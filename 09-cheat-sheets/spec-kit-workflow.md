...
**Spec-Kit is GitHub's official tool for Spec-Driven Development. It enforces the `specify → clarify → plan → tasks → implement` sequence and prevents participants from skipping directly to code without a specification.**
...
| **Expected outcome** | `spec.md`, `plan.md`, and `tasks.md` in `.spec/<NNN>-<feature>/` |
...
| Stage 2 | `/speckit.specify` | `.spec/<NNN>-<feature>/spec.md` |
| Stage 2 | `/speckit.clarify` | Questions resolved in the spec |
| Stage 2 | `/speckit.plan` | `.spec/<NNN>-<feature>/plan.md` |
| Stage 2 | `/speckit.tasks` | `.spec/<NNN>-<feature>/tasks.md` |
...
Upstream commands may be optional in other projects. In this workshop,
`/speckit.clarify` and `/speckit.analyze` are required before the C2 self-check accepts the plan.
Use checklist/issue generation only when it helps the selected work.
...
| 6 | Complex | While `[state]`, when `[event]`, the system SHALL `[one response]` | Combine only clauses needed by the reviewed behavior |
...
```yaml
REQ-NNN:
  source_legacy: <actual supported source path#Lstart-Lend or [GREENFIELD] + justification>
...
Use the workshop-agreed release from <https://github.com/github/spec-kit/releases>, matching the setup guide and installed command help; do not silently upgrade mid-exercise.

```bash
specify init . --integration copilot
```

On macOS/Linux, scripts are stored in `.specify/scripts/bash/`. Features generated by the commands are stored in `.spec/<NNN>-<feature>/`.

> [!NOTE]
> If commands are missing, inspect the installed release's help, generated files
> and workspace root. Initialization is performed once and shared through review;
> do not overwrite existing artifacts by blindly rerunning it.
...
- Before `/speckit.plan`, validate the scope while covering Product Owner and Software Architect responsibilities.
