...
**Complete profile for the Technical Lead persona.** Defines the mission, responsibilities by stage, tools, self-check, and evaluation rubrics.

| Field | Value |
|---|---|
| **Role** | Technical Lead |
| **Role scope** | Technical leadership responsibility (covered by the participant) |
| **Active stages** | Stages 2-3: sequencing, standards, self-review, and submission readiness |
| **Artifacts produced** | Task sequencing, standards review notes, C3 submission readiness checks |
| **Artifacts consumed** | REQ-IDs, ADRs, C4 (Architecture responsibility) |
| **Self-check focus** | C2 task readiness and C3 submission gate |
...
The Technical Lead connects the architecture defined on paper to the code written every day. In the industry, this role defines implementation standards (coding conventions, test style, module structure), unblocks the participant when someone gets stuck on a technical detail, and is accountable for the technical quality of deliveries.

In SIFAP (Payment Inspection and Administration System), the TL ensures that the application created by the participant actually runs end to end by the end of Stage 3 — not merely compiles. This includes decisions such as which layer receives the `@Transactional` annotation, how errors are handled, and how integration tests are structured.
...
flowchart LR
    ...
    E3 --> E4["final validation<br/>final validation"]:::active
```

- **Receives from:** Architecture responsibility (Architecture) in Stage 2 — REQ-IDs + ADRs + C4
- **Hands off to:** Submission support responsibility (Operations) in Stage 3 — running code
...
| **Final judge validation** | Review Copilot's PR line by line before merging. | Production-quality PR |
...
| **Artifact** | Purpose |
|---|---|
| `.github/skills/persona-technical-lead/SKILL.md` | Role skill that loads automatically for technical governance |
...
- **Copilot Chat** for local design decisions and self-review.
- **GitHub Spec-Kit** — support in `/speckit.tasks`, `/speckit.analyze`, and the self-check to `/speckit.implement`.
...
- [ ] **Note the self-check.** Know what DevOps must receive at the end of Stage 3.
...
| Work does not fit the timebox | Review scope and dependencies across the five actual participants. DBA and Developer own their tasks; another participant reviews evidence. Do not invent extra developers or promise a timed merge |
| PR has conflicts | `git pull --rebase` and resolve them. Do not let the branch diverge without aligning with your current role focus |
...
- **Rubric A3 (Technical Integrity):** the application created by the participant runs locally and in CI.
...
| Previous | Next |
|---|---|
| [Software Architect](../04-software-architect/PERSONA.md)<br/><sub>Architecture responsibility · bounded contexts and modules.</sub> | [Developer](../06-developer/PERSONA.md)<br/><sub>Implementation responsibility · Java + Next.js + tests.</sub> |
