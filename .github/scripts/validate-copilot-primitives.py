...
# Feature specifications live in `.spec/`; `specs/` and `.specs/` are retired.
RETIRED_SPEC_ROOT_RE = re.compile(r"(?<![A-Za-z0-9_.\-])\.?specs/(?=<|\d|\*)")
URL_TOKEN_RE = re.compile(r"\S*://\S*")
# Spec-Kit installs its own primitives and templates; validate their wiring, not house style.
SPEC_KIT_RE = re.compile(
    r"^(?:\.specify/|\.github/agents/speckit\.[^/]+\.agent\.md$"
    r"|\.github/prompts/speckit\.[^/]+\.prompt\.md$|\.github/skills/speckit-[^/]+/)"
)


def is_spec_kit(rel: str) -> bool:
    return SPEC_KIT_RE.match(rel) is not None


BUILTIN_AGENT_IDS = {"ask", "agent", "plan", "edit"}
        ...
        for event, handlers in hooks.items():
            ...
            for handler in handlers:
                if not isinstance(handler, dict):
                    reporter.error("hooks", rel, 1,
                                   f"event '{event}' has a non-object handler")
                    continue
    ...
    for i in range(start, len(lines)):
        ...
        if heading:
            title = canonical_section_title(
                ATX_CLOSING_RE.sub("", heading.group(1)).strip())
...
def check_prompt_structure(rel: str, reporter: Reporter) -> None:
    ...
    canonical = {name: index for index,
                 name in enumerate(PROMPT_REQUIRED_SECTIONS)}
    highest_rank = -1
...
def check_stale_paths(text_files: list[str], reporter: Reporter) -> None:
    for rel in text_files:
        ...
        for lineno, line in source:
            ...
            if not rel.startswith("01-archaeology/legacy-sifap/") and RETIRED_SPEC_ROOT_RE.search(
                URL_TOKEN_RE.sub("", line)
            ):
                reporter.error(
                    "policy-stale-path", rel, lineno,
                    "feature specifications live in '.spec/<NNN>-<feature>/'; "
                    "'specs/' and '.specs/' roots are retired",
                )
...
def check_spec_kit_primitives(files: list[str], reporter: Reporter) -> set[str]:
    """Check only what breaks loading: frontmatter, skill names, agent bindings."""
    agent_ids: set[str] = set()
    for rel in files:
        if match(rel, r"\.github/agents/"):
            agent_ids.add(rel.rsplit("/", 1)[-1][: -len(".agent.md")])
    for rel in files:
        if not rel.endswith(".md") or rel.startswith(".specify/"):
            continue
        fm_lines = split_frontmatter(read_text(rel))
        if fm_lines is None:
            if not match(rel, r"\.github/skills/speckit-[^/]+/(?!SKILL\.md$)"):
                reporter.error("spec-kit", rel, 1,
                               "missing or unterminated YAML frontmatter")
            continue
        ...
        if rel.endswith(".prompt.md"):
            agent_value = get_top_value(fm_lines, "agent")
            if agent_value and agent_value not in agent_ids | BUILTIN_AGENT_IDS:
                reporter.warning(
                    "spec-kit", rel, 1,
                    f'agent: "{agent_value}" is not installed; the command does nothing until '
                    "`specify init --here --integration copilot` restores .github/agents/speckit.*",
                )
    return agent_ids
...
def main() -> int:
    ...
    spec_kit_files = [f for f in files if is_spec_kit(f)]
    files = [f for f in files if not is_spec_kit(f)]
    ...
    valid_agent_ids |= check_spec_kit_primitives(spec_kit_files, reporter)
    valid_agent_ids |= BUILTIN_AGENT_IDS
