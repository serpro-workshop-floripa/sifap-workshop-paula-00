...
description: "Use when eliciting, writing, reviewing, or handing off EARS requirements from participant-discovered legacy evidence. Supports official Spec-Kit spec.md, plan.md, tasks.md, REQ-ID and source_legacy traceability, data migration acceptance, and proportionate SDD quality gates."
---
...
- "Prepare implementation-ready checkpoint notes from this approved specification."

## Operating modes

### Binding to this participant kit

The [kit workflow](../../../09-cheat-sheets/spec-kit-workflow.md) is authoritative:

- Stage 1 generates evidence through guided reading; it does not receive prefilled catalogues, mystery solutions, target models, or invented approvals.
- Read the actual legacy sources before EARS. Keep unresolved mysteries unconfirmed; no requirement is derived from an unanswered question.
- Every feature lives in `.spec/<NNN>-<feature>/`, pinned in `.specify/feature.json` so the `/speckit.*` commands resolve it. The upstream `/speckit.specify` script writes to a dot-less root; move its output into `.spec/` and never keep two trees. The [SDD artifacts instruction](../../instructions/sdd-artifacts.instructions.md) owns the layout and file contracts.
- All new normative requirements use `REQ-NNN` and `source_legacy:` with a real supported source path or `[GREENFIELD]` plus the team's justification. `SRC-###` is supplementary provenance, never a replacement.
- Emit `source_legacy:` unbulleted within the next 20 lines after each requirement declaration and before the next one; a Markdown list item is not accepted by the current CI parser. Declare requirements only in `spec.md`; elsewhere cite REQ-IDs in table cells or mid-sentence, never at the start of a line.
- Preserve existing requirement and acceptance IDs. New acceptance IDs use `AC-REQ-NNN-NN`; FR/NFR labels describe a category field, not a competing identifier scheme.
- The ten logical responsibilities in the bundled references map to lowercase Spec-Kit files, never to uppercase files: specification and source traceability to `spec.md`; analysis and decisions to `research.md`; design to `plan.md`, `data-model.md`, and `contracts/`; testing to `quickstart.md` and the test map in `tasks.md`; tasks, cross-analysis, and verification to `tasks.md`; checklists to `checklists/`. Link ADRs, scope decisions, and [data records](../../../docs/DATA-MIGRATION.md) instead of copying them.
- A feature is ready for C2 only when its complete artifact set exists, with non-applicable files stating why. Narrower modes update only the files they touch; no generated sign-off or mandatory diagram quota.
- Use only validation tools actually present. This kit has [spec-quality.yml](../../workflows/spec-quality.yml), `/speckit.analyze`, `/speckit.checklist`, and [primitive validation](../../scripts/validate-copilot-primitives.py), not the generic `scripts/validate-sdd-*` tools.
...
1. Establish scope and action.
   ...
   - Identify whether the request is requirements-first, design-first, validation-only, or checkpoint-only.
...
4. Author or normalize requirements.
   ...
   - Assign stable `REQ-NNN` IDs and acceptance IDs compatible with the existing feature; record functional/non-functional category separately.
   - Add `source_legacy:` within the CI-supported distance of each requirement declaration; supplementary source IDs do not replace it.
...
5. Build the SDD artifact chain.
   ...
   - Reuse `.specify/memory/constitution.md`. Do not create a feature-local constitution.
   - For full SDD, create or update the complete set in `.spec/<NNN>-<feature>/`: `spec.md`, `research.md`, `plan.md`, `data-model.md`, `contracts/`, `quickstart.md`, `tasks.md`, and `checklists/`, stating non-applicability with a reason instead of omitting a file.
   ...
   - Use the next free zero-padded `.spec/<NNN>-<feature>` number and the matching `spec/<NNN>-<feature>` branch.

6. Enforce end-to-end traceability.
   - Give every active requirement one explicit source-traceability row in `spec.md`.
   - Map each active requirement to design components in `plan.md`, entities in `data-model.md` when data is involved, tasks and tests in `tasks.md`, and a validation scenario in `quickstart.md`.
...
7. Validate and hand off.
   ...
   - Run the existing applicable checks from `spec-quality.yml`. Inspect named commands before use; missing tooling is reported as unavailable, not invented or installed just to mimic another framework.
...
| Field | Rule |
| --- | --- |
| ID | Stable, unique `REQ-NNN`; category/domain recorded separately |
...
| Source | Mandatory `source_legacy:` with actual source or justified `[GREENFIELD]`; optional supplementary `SRC-###` |
| Rationale | Why the behavior or quality constraint is needed |
| Acceptance | At least one pass/fail signal |
| Verification | Planned test, inspection, analysis, or measurement; no workshop demo deliverable |
...
- [Unified quality gates](references/quality-gates.md): detailed readiness, EARS, traceability, and checkpoint checks.
...
- [ ] The design in `plan.md` maps requirements through components, tasks, dependencies, tests/evidence, and current-versus-target state.
- [ ] `tasks.md` checkboxes, dependencies, test map, and verification ledger agree; partial implementation is not checked as complete.
...
- [ ] Existing applicable kit checks ran, or unavailable checks are explicitly reported without a success claim.
- [ ] `.spec/`, the complete lowercase artifact set, `REQ-NNN`, `source_legacy:`, and participant-only scope are preserved; no uppercase package, second tree, or worked solution is generated.
