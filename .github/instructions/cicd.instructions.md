...
| `spec-quality.yml` · `spec-traceability` | Reports REQ-IDs in `.spec/` not yet referenced by a test (emits `::warning::`) | No |
| `spec-quality.yml` · `legacy-traceability` | Every REQ-ID in `.spec/` must carry a valid `source_legacy:` line | Yes |

> [!IMPORTANT]
> `legacy-traceability` fails the build; `spec-traceability` only warns. See [`spec-quality.yml`](../workflows/spec-quality.yml) for the exact `source_legacy:` format the gate accepts and the EARS validation skill for requirement authoring.
